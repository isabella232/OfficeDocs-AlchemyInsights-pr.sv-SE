---
title: '8000020: exportera kontakter'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: 311964b1e8e049ad7c26b106d5fe551f2d348170
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694221"
---
# <a name="export-contacts"></a>Exportera kontakter

1. Längst ned på sidan i Outlook på webben väljer du <img src='data:image/png;base64,' />
 **Personer**.
2. I verktygsfältet väljer du **Hantera**  >  **Exportera kontakter**. 
3. Välj att exportera alla kontakter eller bara kontakter från en särskild mapp och välj sedan **Exportera**.
4. Längst ned på sidan väljer du **Spara** för att spara contacts.csv i standardmappen. Eller Välj pilen bredvid **Spara** och välj **Spara som**.

Mer information finns i [Exportera kontakter från Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

