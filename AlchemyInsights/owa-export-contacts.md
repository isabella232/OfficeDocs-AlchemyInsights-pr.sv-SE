---
title: '8000020: Exportera kontakter'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: d7dfdf30ec51f393cb47cb3fe89577923d4ac054579391f1fa9700bb9c3c6319
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983291"
---
# <a name="export-contacts"></a>Exportera kontakter

1. Längst ned på sidan i Outlook på webben väljer du <img src='data:image/png;base64,' />
 **People**.
2. Välj Hantera exportera kontakter  >  **i verktygsfältet.** 
3. Välj om du vill exportera alla kontakter eller bara kontakter från en särskild mapp och välj sedan **Exportera**.
4. Längst ned på sidan väljer du Spara **för** att spara "contacts.csv" i standardmappen för nedladdningar. Du kan också välja pilen bredvid **Spara** och välja **Spara som**.

Mer information finns i [Exportera kontakter från Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

