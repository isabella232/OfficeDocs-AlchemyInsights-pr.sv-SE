---
title: Exportera kontakter på Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "279"
- "8000020"
ms.assetid: 37fc1455-7324-4c76-9a94-085a45e94c51
ms.openlocfilehash: 9f8a3c2ad2b2d6736d34f168bc5733d554ee32e95d8ea6f12069feea1a12ec49
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: sv-SE
ms.lasthandoff: 08/05/2021
ms.locfileid: "54024692"
---
# <a name="export-your-contacts-in-outlookcom"></a>Exportera dina kontakter på Outlook.com

1. Gå till [Personer](https://outlook.live.com/people/) <img src='data:image/png;base64,' />
.
2. Högst upp på sidan väljer du **Hantera** \> **export.**
3. Välj om du vill exportera alla kontakter eller bara kontakter från en särskild mapp och välj **Exportera**.
4. Längst ned på sidan väljer du **Spara så** sparas "contacts.csv" i standardmappen för nedladdningar. Om du vill välja var du .csv filen ska sparas väljer du pilen bredvid Spara och sedan **Spara som**.

Mer information finns i Exportera [kontakter från Outlook.com till en CSV-fil.](https://support.office.com/article/578cca22-3550-4c73-b3f0-9978cfeac83f?wt.mc_id=Office_Outlook_com_Alchemy)